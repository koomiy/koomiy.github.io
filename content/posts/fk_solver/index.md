...
今回はvnoidパッケージの機能の一つ、順運動学ソルバー(fksilver)について解説します。

---

## 順運動学とは
順運動学とは、ロボットにある関節変位(ここでは回転のみ扱います)を与えたときの  
...
上記の議論に基づけば、 $\Sigma_{0}$ から見た $\Sigma_{i}$ の位置は次式で表せます。  
...
一方、$\Sigma_{0}$ から見た $\Sigma_{i}$ の姿勢は次式で表せます。  
$$ \boldsymbol{{}^0R_i} = \boldsymbol{{}^0R_{i-1}} \boldsymbol{{}^{i-1}R_i} $$

i = 1から先端効果器を意味する番号までこれらの式を用いれば、  
...
-	**足裏中心の位置、姿勢を計算(132~134行目)**
	...
	ベースリンクから腰関節までのベクトルを`base_to_hip`、  
	くるぶしから足裏中心までのベクトルを`ankle_to_foot`とします。  
